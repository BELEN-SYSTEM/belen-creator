...
PropietariosPage::PropietariosPage(SupabaseClient* supabase, HistorialService* historial,
                                   QWidget* parent)
    : QWidget(parent)
    , m_supabase(supabase)
    , m_service(new PropietarioService(supabase, historial, this))
{
    auto* layout = new QVBoxLayout(this);
    layout->setContentsMargins(24, 24, 24, 24);

    auto* title = new QLabel(tr("Propietarios"), this);
    title->setStyleSheet(QStringLiteral("font-size: 22px; font-weight: bold; color: #2c3e50;"));
    layout->addWidget(title);

    auto* desc = new QLabel(tr("Gestiona los propietarios de piezas"), this);
    ...
    connect(addBtn, &QPushButton::clicked, this, &PropietariosPage::onAdd);
    connect(m_editBtn, &QPushButton::clicked, this, &PropietariosPage::onEdit);
    connect(m_deleteBtn, &QPushButton::clicked, this, &PropietariosPage::onDelete);
    ...
    connect(m_service, &PropietarioService::dataReady, this, [this](const QVector<Propietario>& list) {
    ...
    connect(m_service, &PropietarioService::mutationDone, this, &PropietariosPage::refreshTable);
    connect(m_service, &PropietarioService::errorOccurred, this, [this](const QString& msg) {
        m_overlay->hideOverlay();
        QMessageBox::critical(this, tr("Propietarios"), msg);
    });

    refreshTable();
}

void PropietariosPage::refreshTable()
{
    m_overlay->showOverlay();
    m_service->fetchAll();
}

void PropietariosPage::onAdd()
{
    QDialog dlg(this);
    dlg.setWindowTitle(tr("A\u00f1adir propietario"));
    auto* form = new QFormLayout(&dlg);
    auto* nombreEdit = new QLineEdit(&dlg);
    nombreEdit->setPlaceholderText(tr("Nombre del propietario"));
    ...
    if (nombre.isEmpty()) {
        QMessageBox::warning(this, tr("Propietarios"), tr("El nombre no puede estar vac\u00edo."));
        return;
    }
    m_overlay->showOverlay();
...
void PropietariosPage::onEdit()
{
    ...
    QDialog dlg(this);
    dlg.setWindowTitle(tr("Editar propietario"));
    auto* form = new QFormLayout(&dlg);
    auto* nombreEdit = new QLineEdit(&dlg);
...
void PropietariosPage::onDelete()
{
    int id = selectedId();
    if (id <= 0) return;
    QString nombre = m_table->item(m_table->currentRow(), 1)->text();
    auto ret = QMessageBox::question(this, tr("Eliminar propietario"),
...
int PropietariosPage::selectedId() const
{
    int row = m_table->currentRow();
    if (row < 0) return 0;
